...
@Component({
  selector: 'walletviptable',
  templateUrl: './walletvip-table.component.html',
  styleUrls: ['./walletvip-table.component.scss'],
  animations: [
    flyIn
  ]
})
export class WalletvipComponent implements OnInit {
    ...
    ngOnInit() {
       this.down = this.constant.root+'/workspace/operating/walletviptable';
    }
    public getByPage(page: number) {
     ...
     let opt = {
         ...
         url:_this.constant.root+"ydmgmt/api/statistics/queryVipWalletData",
         type:"get"
       }
       this.down = this.constant.root+"ydmgmt/api/statistics/exportVipWalletData?"+this.constant.getParams(opt.req);
