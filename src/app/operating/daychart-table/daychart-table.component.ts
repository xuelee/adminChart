...
@Component({
  selector: 'daycharttable',
  templateUrl: './daychart-table.component.html',
  styleUrls: ['./daychart-table.component.scss'],
  animations: [
    flyIn
  ]
})
...
export class DaychartComponent implements OnInit {
    ...
    ngOnInit() {
      ...
      this.down = this.constant.root+"ydmgmt/api/statistics/exportDayStatisYY?"+this.constant.getParams(req);
    }
    public getByPage(page: number): void {
     ...
     let opt = {
         ...
         url:_this.constant.root+"ydmgmt/api/statistics/queryDayStatisYY",
         type:"get"
       }
       this.down = this.constant.root+"ydmgmt/api/statistics/exportDayStatisYY?"+this.constant.getParams(opt.req);
